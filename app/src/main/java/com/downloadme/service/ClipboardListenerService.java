...
public class ClipboardListenerService extends Service {
    ...
    SharedPreferences one_play_preferences, one_play_preferences2;
    SharedPreferences.Editor one_play_editor, one_play_editor2;
    int coin;
    Boolean coin_alfa;
        ...
        protected void onPostExecute(String jsonStr) {
            if (jsonStr != null) {
                ...
                try {
                    ...
                    JSONObject reel = jsonObject.getJSONObject("reel");

                    if (Objects.equals(status, "ok")) {

                        if (reel != null) {

                            JSONArray item = reel.getJSONArray("items");

                            for (int i = 0; i < item.length(); i++) {

                                JSONObject t = item.getJSONObject(i);
                                int media_type = t.getInt("media_type");

                                if (media_type == 1) {
                                    //photo
                                    JSONObject image_versions2 = t.getJSONObject("image_versions2");
                                    JSONArray candidates = image_versions2.getJSONArray("candidates");
                                    JSONObject photo1 = candidates.getJSONObject(1);
                                    url2 = photo1.getString("url");
                                    tvp = "i";
                                } else {
                                    //video
                                    JSONArray image_versions2 = t.getJSONArray("video_versions");
                                    JSONObject photo1 = image_versions2.getJSONObject(0);
                                    url2 = photo1.getString("url");
                                    tvp = "v";
                                }

                                new StoryDownloader(getApplicationContext()).download(url2, tvp, "_" + i, id_org, item.length());
                                ...
                                if (jj == i) {
                                    ...
                                    mBuilder.setContentTitle("Stories Downloaded!");
                                    mBuilder.setSmallIcon(R.drawable.notif_tick);
                                    mBuilder.setContentText("Download Stories " + " " + id_org + " " + " Was Successful");
                                    mNotifyManager.notify(35000, mBuilder.build());
                                }
                            }
                        } else {
                            Toast.makeText(getApplicationContext(), "Not Stories", Toast.LENGTH_LONG).show();
                        }
                    } else {
                        Toast.makeText(getApplicationContext(), "Not Stories", Toast.LENGTH_LONG).show();
                    }
    ...
    public void downloadprofile() {
        if (pb == null) {
            Toast.makeText(getApplicationContext(), "Profile picture failed to download", Toast.LENGTH_SHORT).show();
        } else {
            SaveImage(pb);
            ...
            mBuilder1.setContentTitle("Download Profile Successful");
            mBuilder1.setSmallIcon(R.drawable.notif_tick);
            mBuilder1.setContentText("Profile picture downloaded");
    ...
    class C16431 implements OnPrimaryClipChangedListener {
        ...
        public void onPrimaryClipChanged() {
            ...
            if (a == 0) {
                ...
                Toast.makeText(getApplicationContext(), "That Instagram not!", Toast.LENGTH_SHORT).show();
            }
        }
    }
        ...
        try {
            ...
            try {
                ...
                Intent myAppLinkToMarket2 = new Intent(Intent.ACTION_VIEW, Uri.parse("http://play.google.com/store/apps/details?id="
                        + getPackageName()));
                PendingIntent pendingIntent2 = PendingIntent.getActivity(this, 0, myAppLinkToMarket2, 0);
                pendingIntent2.send();
    ...
    private void m7902b(final int i) {
        ...
        one_play_preferences2 = getApplicationContext().getSharedPreferences("PROJECT_NAME", android.content.Context.MODE_PRIVATE);
        one_play_editor2 = one_play_preferences2.edit();
        coin = one_play_preferences2.getInt("COIN", 0);
        if (i == 1) {
            if (coin_alfa) {
                ...
                if (start) {
                    Toast.makeText(getApplicationContext(), "Download Stories Start", Toast.LENGTH_SHORT).show();
                }
                C03611();
                ...
                one_play_editor.apply();
                if (coin <= 990) {
                    Boolean one_riz = one_play_preferences.getBoolean("Comment_Google", true);
                    if (one_riz) {
                        one_play_editor.putBoolean("Comment_Google", false);
                        one_play_editor.apply();
                        m68();
                    }
                }
            ...
            if (coin_alfa) {
                if (start) {
                    Toast.makeText(getApplicationContext(), "Download Profile Start", Toast.LENGTH_SHORT).show();
                }
    ...
    public void C03611() {
        ...
        try {
            ...
            Toast.makeText(getApplicationContext(), "Error Download Stories!", Toast.LENGTH_SHORT).show();
