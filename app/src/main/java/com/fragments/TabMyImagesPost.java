...
import com.adapters.MySavedImagesAdapter_Post;
import com.adapters.MySavedImagesAdapter_Post.onItemClickListener;
import com.adapters.MySavedImagesAdapter_Post.onItemLongClickListener;
...
import com.fury.instafull.ViewImageActivityPost;
...
public class TabMyImagesPost extends Fragment implements onItemLongClickListener, onItemClickListener {
    public static TabMyImagesPost act;
    public static ArrayList<DownloadModel_Post> items;
    private String currentPath;
    private boolean doCut;
    private boolean isSelecting;
    private MySavedImagesAdapter_Post mAdapter;
    Context mContext;
    Menu menu;
    ...
    class C03962 extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            ...
            if (files != null) {
                for (File path : files) {
                    items.add(new DownloadModel_Post(path.getAbsolutePath()));
                }
            }
            return null;
        }
        ...
        protected void onPostExecute(Void result) {
            ...
            mAdapter = new MySavedImagesAdapter_Post(getActivity(), items);
            recyclerView.setLayoutManager(new GridLayoutManager(mContext, 3));
            recyclerView.setHasFixedSize(true);
            recyclerView.setAdapter(mAdapter);
            mAdapter.setOnItemClickListener(TabMyImagesPost.this);
            mAdapter.setOnItemLongClickListener(TabMyImagesPost.this);
            checkListSize();
    ...
    class C04004 implements OnClickListener {
        ...
        class C03992 extends AsyncTask<Void, Void, Void> {
            ...
            protected Void doInBackground(Void... params) {
                Bitmap bitmap = BitmapFactory.decodeFile(((DownloadModel_Post) TabMyImagesPost.items.get(this.val$position)).filePath);
                WallpaperManager wallpaperManager = WallpaperManager.getInstance(getActivity());
                try {
                    wallpaperManager.setBitmap(bitmap);
                    wallpaperManager.suggestDesiredDimensions(Utils.f100w, Utils.f99h);
                } catch (IOException e) {
                    FirebaseCrash.report(new Exception("C03992"));
                }
                return null;
            }
        ...
        public void onClick(DialogInterface dialog, int which) {
            switch (which) {
                case R.styleable.View_android_theme /*0*/:
                    String path = ((DownloadModel_Post) items.get(val$position)).filePath;
                ...
                case R.styleable.View_android_focusable /*1*/:
                    ...
                    share.setType("image/*");
                    share.putExtra("android.intent.extra.STREAM", Uri.fromFile(new File(((DownloadModel_Post) items.get(this.val$position)).filePath)));
                    startActivity(Intent.createChooser(share, "Share Picture Via.."));
                    FirebaseCrash.log("C04004 1");
                    break;
                case R.styleable.View_paddingStart /*2*/:
                    try {
                        createInstagramIntent("image/*", ((DownloadModel_Post) items.get(this.val$position)).filePath);
                    } catch (Exception e) {
                ...
                case R.styleable.View_paddingEnd /*3*/:
                    DialogUtils.showDeleteDialog(Post_view.act, new C03981(this.val$position));
    ...
    class deleteSingleFilesAsync extends AsyncTask<Void, Integer, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            DownloadModel_Post model = (DownloadModel_Post) items.get(this.pos);
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        this.currentPath = Environment.getExternalStorageDirectory().toString() + "/" + name_pic;
    ...
    private void checkListSize() {
        ...
        this.tvLoading.setText("No Image \n You have no image download");
    ...
    public void onItemLongClick(int position) {
        ...
        CharSequence[] colors = new CharSequence[]{"Details", "Share", "Write for posts", "Remove", "Wallpaper"};
    ...
    public void onItemClick(int position) {
        Intent it = new Intent(this.mContext, ViewImageActivityPost.class);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 215 && resultCode == -1) {
            if (this.doCut) {
                ArrayList<DownloadModel_Post> listSelected = this.mAdapter.getSelectedModel();
