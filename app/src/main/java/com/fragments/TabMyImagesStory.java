...
import com.adapters.DownloadModel_Story;
import com.adapters.MySavedImagesAdapter_Story;
import com.adapters.MySavedImagesAdapter_Story.onItemClickListener;
import com.adapters.MySavedImagesAdapter_Story.onItemLongClickListener;
import com.adapters.SpacesItemDecoration_Post;
...
import com.fury.instafull.Story_view;
import com.fury.instafull.ViewImageActivityStory;
...
public class TabMyImagesStory extends Fragment implements onItemLongClickListener, onItemClickListener {
    public static TabMyImagesStory act;
    public static ArrayList<DownloadModel_Story> items;
    private String currentPath;
    private boolean doCut;
    private boolean isSelecting;
    private MySavedImagesAdapter_Story mAdapter;
    Context mContext;
    Menu menu;
    ...
    /* renamed from: com.fragments.TabMyImages.1 */
    ...
    class C03962 extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            ...
            if (files != null) {
                for (File path : files) {
                    TabMyImagesStory.items.add(new DownloadModel_Story(path.getAbsolutePath()));
                }
            }
            return null;
        }
        ...
        protected void onPostExecute(Void result) {
            ...
            mAdapter = new MySavedImagesAdapter_Story(getActivity(), items);
            recyclerView.setLayoutManager(new GridLayoutManager(mContext, 3));
            recyclerView.setHasFixedSize(true);
            recyclerView.setAdapter(mAdapter);
            mAdapter.setOnItemClickListener(TabMyImagesStory.this);
            mAdapter.setOnItemLongClickListener(TabMyImagesStory.this);
            checkListSize();
            super.onPostExecute(result);
        }
    }

    /* renamed from: com.fragments.TabMyImages.3 */
    class C03973 implements OnKeyListener {
        C03973() {
        }
    ...
    class C04004 implements OnClickListener {
        ...
        class C03981 implements DialogBtnClickListener {
            ...
            public void onPositiveClick() {
                new deleteSingleFilesAsync(this.val$position).execute(new Void[0]);
            }
        }

        /* renamed from: com.fragments.TabMyImages.4.2 */
        class C03992 extends AsyncTask<Void, Void, Void> {
            private final /* synthetic */ int val$position;
            ...
            protected Void doInBackground(Void... params) {
                Bitmap bitmap = BitmapFactory.decodeFile(((DownloadModel_Story) TabMyImagesStory.items.get(this.val$position)).filePath);
                WallpaperManager wallpaperManager = WallpaperManager.getInstance(getActivity());
                try {
                    wallpaperManager.setBitmap(bitmap);
                    wallpaperManager.suggestDesiredDimensions(Utils.f100w, Utils.f99h);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                return null;
        ...
        public void onClick(DialogInterface dialog, int which) {
            switch (which) {
                case R.styleable.View_android_theme /*0*/:
                    ...
                    share.setType("image/*");
                    share.putExtra("android.intent.extra.STREAM", Uri.fromFile(new File(((DownloadModel_Story) items.get(this.val$position)).filePath)));
                    startActivity(Intent.createChooser(share, "Share Picture Via.."));
                    FirebaseCrash.log("C04004 0");
                    break;
                case R.styleable.View_android_focusable /*1*/:
                    try {
                        createInstagramIntent("image/*", ((DownloadModel_Story) items.get(this.val$position)).filePath);
                    } catch (Exception e) {
                ...
                case R.styleable.View_paddingStart /*2*/:
                    DialogUtils.showDeleteDialog(Story_view.act, new C03981(this.val$position));
                    FirebaseCrash.log("C04004 2");
                    break;
                case R.styleable.View_paddingEnd /*3*/:
                    try {
                        new C03992(this.val$position).execute();
                    } catch (Exception e) {
                        FirebaseCrash.report(new Exception("set background"));
                    }
                    FirebaseCrash.log("C04004 3");
                    break;
            }
        }
    }

    /* renamed from: com.fragments.TabMyImages.5 */
    class C04015 implements DialogBtnClickListener {
        private final /* synthetic */ ArrayList val$listElements;

        C04015(ArrayList arrayList) {
    ...
    class C04026 implements View.OnClickListener {
        ...
        public void onClick(View rippleView) {
            ...
            try {
                ...
                DialogUtils.showUpdateDialog(Story_view.act);
    ...
    class deleteFilesAsync extends AsyncTask<Void, Integer, Void> {
        ArrayList<DownloadModel_Story> listItems;
        ProgressDialog pd;

        public deleteFilesAsync(ArrayList<DownloadModel_Story> listItems) {
            this.listItems = listItems;
        }

        protected void onPreExecute() {
            this.pd = new ProgressDialog(Story_view.act);
        ...
        protected Void doInBackground(Void... arg0) {
            ...
            while (it.hasNext()) {
                DownloadModel_Story model = (DownloadModel_Story) it.next();
                File f = new File(model.filePath);
                if (f.delete()) {
                    items.remove(model);
                    Utils.scanDeletedMedia(mContext, f, "image/*");
                }
                value++;
        ...
        protected void onPostExecute(Void result) {
            try {
                ...
                isSelecting = false;
                checkListSize();
                mAdapter.removeItems(listItems);
                mAdapter.resetAdapter();
            } catch (Exception e) {
    ...
    class deleteSingleFilesAsync extends AsyncTask<Void, Integer, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            DownloadModel_Story model = (DownloadModel_Story) items.get(this.pos);
            if (new File(model.filePath).delete()) {
                Utils.scanDeletedMedia(getActivity(), new File(model.filePath), "image/*");
            }
            return null;
        }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        this.currentPath = Environment.getExternalStorageDirectory().toString() + "/" + name_image;
        this.recyclerView = (RecyclerView) v.findViewById(R.id.recycler_view);
        this.recyclerView.addItemDecoration(new SpacesItemDecoration_Post(2));
        new C03962().execute();
        return v;
    }
    ...
    private void checkListSize() {
        ...
        this.tvLoading.setText("No Image Story \n" +
                "You have no image story download");
    ...
    public void onResume() {
        ...
        getView().setOnKeyListener(new C03973());
        super.onResume();
    }
    ...
    public void onItemLongClick(int position) {
        ...
        CharSequence[] colors = new CharSequence[]{"Share", "Write for posts", "Remove", "Wallpaper"};
    ...
    public void onItemClick(int position) {
        Intent it = new Intent(this.mContext, ViewImageActivityStory.class);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 215 && resultCode == -1) {
            if (this.doCut) {
                ArrayList<DownloadModel_Story> listSelected = this.mAdapter.getSelectedModel();
