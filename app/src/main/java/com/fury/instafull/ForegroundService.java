...
public class ForegroundService extends Service {
    ...
    private class fetchUrl extends AsyncTask<String, Void, String> {
        ...
        class C15451 implements FileDownloadFromServiceListener {
            ...
            public void onFileDownloadComplete(int NotifId) {
                ...
                if (End){
                    Toast.makeText(ForegroundService.this.getApplicationContext(), new StringBuilder(String.valueOf(ForegroundService.this.fileType == 0 ? "Photo" : "Video")).append(" has been saved!").toString(), Toast.LENGTH_SHORT).show();
                }
                fetchUrl.this.mBuilder.setContentTitle("Downloaded!");
                fetchUrl.this.mBuilder.setSmallIcon(R.drawable.notif_tick);
                fetchUrl.this.mBuilder.setContentText("Download was successful");
            ...
            public void onConnectionError(int NotifId) {
                Toast.makeText(ForegroundService.this.getApplicationContext(),"Internet access is down!", Toast.LENGTH_SHORT).show();
        ...
        protected void onPostExecute(String url) {
            ...
            if (url.length() < 10) {
                Toast.makeText(ForegroundService.this.getApplicationContext(), "User page private is!", Toast.LENGTH_SHORT).show();
                return;
            }
            try {
            ...
            try {
                ...
                this.mBuilder.setContentTitle("Are download ...").setContentIntent(pendingIntent).setContentText("Getting "+ (ForegroundService.this.fileType == 0 ? "picture": "video")).setSmallIcon(R.drawable.download_icon);
                ...
                Toast.makeText(ForegroundService.this.getApplicationContext(), "An error in to download occurred!", Toast.LENGTH_SHORT).show();
    ...
    private void showNotification() {
        ...
        if (show){
            startForeground(R.styleable.AppCompatTheme_buttonStyleSmall, new Builder(this).setContentTitle("Instafull is on").setTicker("Time to download!").setContentText("Download now fast and easy!").setSmallIcon(R.drawable.notif_icon).setLargeIcon(Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.ic_launcher), 60, 60, false)).setContentIntent(pendingIntent).setOngoing(true).addAction(R.drawable.stop, "Instafull disabled", PendingIntent.getBroadcast(this, 0, new Intent(ACTION.STOPFOREGROUND_ACTION), 0)).build());
        }
